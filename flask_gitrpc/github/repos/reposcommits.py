...
class ReposCommits:
  ...
  def list_commits(self, repo, sha=None, path=None, user=None):
    ...
    url = 'repos/%s/%s/commits' % (self.client.username(user), repo)
    if sha and path:
      # todo: urlencode params?
      url = 'repos/%s/%s/commits?%s' % (
        self.client.username(user), repo, params)
    return self.client.get(url, RepoCommitListResponse)

  def get_commit(self, repo, sha, user=None):
    return self.client.get(
      'repos/%s/%s/commits/%s' % (
        self.client.username(user), repo, sha), RepoCommitResponse)

  def list_repo_comments(self, repo, user=None):
    return self.client.get(
      'repos/%s/%s/comments' % (
        self.client.username(user), repo), RepoCommitListResponse)

  def list_comments(self, repo, sha, user=None):
    return self.client.get(
      'repos/%s/%s/comments/%s/comments' % (
        self.client.username(user), repo, sha), RepoCommitListResponse)
  ...
  def create_comment(self, repo, sha, body, line, path, position, user=None):
    ...
    return self.client.post(
      'repos/%s/%s/commits/%s/comments' % (
        self.client.username(user), repo, sha), msg)
  ...
  def compare_commits(self, repo, base, head, user=None):
    return self.client.get(
      'repos/%s/%s/compare/%s...:%s' % (
        self.client.username(user), repo, base, head))
